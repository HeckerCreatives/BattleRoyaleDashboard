  ...
  // Handle checkbox change
    ...
    useEffect(() =>{
        const getlist = async () => {
            setListload(true)
            try {
                const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/investor/getinvestorlist?page=${currentpage}&limit=10`,{
    ...
    const unsubscribedUsers = async () =>{
       ...
       if(selectedIds.length === 0){
        ...
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/investor/deletemultipleinvestors`,{
