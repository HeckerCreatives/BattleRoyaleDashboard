...
export function EditItemDialog({ trigger, initialData, onSuccess, onSave }: MarketplaceItemDialogProps) {
  ...
  const [formData, setFormData] = useState<MarketplaceItem>({
    ...
    type: initialData?.type?.toLowerCase() || "title",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.itemid || !formData.itemname || !formData.type || formData.amount <= 0 ) return  toast({
             variant: "destructive",
             title: `Error`,
    ...
    try {
      ...
      await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/marketplace/admin/update`, formData, {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="gap-1 bg-blue-600">
         <SquarePen size={15}/>
        </Button>
      </DialogTrigger>
